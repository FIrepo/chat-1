...
describe('friends test', () => {
  ...
  it('should be 1 friend', function() {
    return this.agent.get(api.listFriends('BarbaraPalvin')).expect(200)
    .then((res) => {
      expect(res.body).to.be.ok;
      expect(res.body.data.friends.length).to.equal(1);
    });
  });
  it('should be 0 friend', function() {
    return co(function * () {
      ...
      const res = yield this.agent.get(api.listFriends()).expect(200);
      expect(res.body).to.be.ok;
      expect(res.body.data.friends.length).to.equal(0);
    }.bind(this));
  });
});
